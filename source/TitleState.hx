...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		if(ClientPrefs.checkForUpdates && !closedState) {
			...
			// for in-develop build stuff
			var idkWhy:Bool = true;
			...
			http.onData = function (data:String)
			{
				...
				if(updateVersion != curVersion && !idkWhy) {
					trace('versions arent matching!');
					mustUpdate = true;
					OutdatedState.exWarm = false;
				} else if (updateVersion != curVersion && idkWhy) {
					trace('version arent macthing! but something wrong');
					mustUpdate = true;
					OutdatedState.exWarm = true;
				}
			}
